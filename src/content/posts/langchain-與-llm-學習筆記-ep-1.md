...
**跑起來就是這樣不留情面**——這題撈到的 4 個 chunk 剛好都沒踩中免稅額條件的段落，`llama3.1` 也很老實地不瞎掰。這正是 Naive RAG 的真實樣子：連前處理、後處理都還沒加的 baseline，撈錯就是撈錯。下一篇 [Ep-2](/posts/langchain-與-llm-學習筆記-ep-2) 會把這個坑補起來。
