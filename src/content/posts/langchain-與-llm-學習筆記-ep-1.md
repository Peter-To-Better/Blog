...
description: "從零理解 RAG（檢索增強生成）的完整技術架構，拆解資料索引與查詢生成兩大階段，包含載入、分割、嵌入、索引、檢索與生成的每個步驟。"
author: "Peter"
tags: ["LangChain & LLM"]
...
![RAG Workflow](/public/images/RAG-work-flow.jpg)
...
整個流程可以拆成兩個大階段：**階段一是「資料索引」**，屬於離線（offline）作業，目的是把知識庫預先處理好、存進向量資料庫；**階段二是「查詢與生成」**，屬於線上（online）作業，是使用者每次提問時即時發生的流程。

---

## 階段一：資料索引（離線建立知識庫）

> 載入 → 分割 → 嵌入 → 索引。這一階段只需在資料更新時執行，產出的成果會被後續查詢重複使用。
...
> 檢索前處理 → 檢索 → 檢索後處理 → 生成。這一階段在使用者每次提問時即時執行。
