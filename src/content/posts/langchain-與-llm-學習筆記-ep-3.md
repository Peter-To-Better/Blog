...
換上去之後，同樣的 context、同樣的問題，馬上答對「彰化縣」。兩句話語意幾乎一樣，但前者「明確要求覆誦某句拒答話術」這個寫法，會讓 `llama3.1` 對中文指令更容易過度保守、傾向直接套用那句話收場，即使答案就在眼前。**這是這篇唯一改動 prompt 措辭、其他邏輯完全沒動就修好的坑**，也是為什麼上面的程式碼用的是 Ep-1 那句，不是最初寫的那句。
...
`qwen2.5vl:7b` 把地圖轉述成文字、`nomic-embed-text` 把它跟同頁的圖說一起嵌入、`llama3.1` 從檢索到的 9 份文件裡揪出正確答案，**這次是真的搞定了，不是憑空宣稱。**

> 📌 **後續修訂（寫於 [Ep-4](/posts/langchain-與-llm-學習筆記-ep-4) 實測之後）**：這裡原本寫的是「一個純文字 RAG（沒有這張地圖轉述）永遠答不出來的問題」，**那句話是錯的**。Ep-4 把變因拆開來一組一組測，結果發現：只要把 `top_k` 調到 9，一個**完全沒有圖片轉述**的純文字 RAG 也答得出彰化縣。因為「彰化縣 1.05 居冠」這句話本來就是第 0 頁的純文字，跟地圖無關。真正救回這題的是 `top_k`，不是圖片轉述。
>
> 圖片轉述當然有它不可取代的地方，只是**不在這一題**。Ep-4 找到了真正只有靠圖才答得出來的題目。細節見 [Ep-4：RAGAS 指標可信度實測](/posts/langchain-與-llm-學習筆記-ep-4)。
...
> 中途跳進來的話，系列的順序是 [Ep-0 概念打底](/posts/langchain-與-llm-學習筆記-ep-0) → [Ep-1 RAG 入門](/posts/langchain-與-llm-學習筆記-ep-1) → [Ep-2 進階檢索](/posts/langchain-與-llm-學習筆記-ep-2) → **Ep-3（本篇）** → [Ep-4 評估](/posts/langchain-與-llm-學習筆記-ep-4)。
